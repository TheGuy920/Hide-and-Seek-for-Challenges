...
ChallengeWorld.cellMinX = -20
ChallengeWorld.cellMaxX = 20
ChallengeWorld.cellMinY = -20
ChallengeWorld.cellMaxY = 20
...
function ChallengeWorld.server_onDestroy( self )
	for _,player in pairs(sm.player.getAllPlayers()) do
		sm.event.sendToPlayer(player, "server_setSpectate", {state=false})
	end
end
...
function ChallengeWorld.client_onCreate( self )
	...
	if g_survivalHud and sm.exists(g_survivalHud) and not g_survivalHud:isActive() then
		g_survivalHud:open()
	end
end
...
function ChallengeWorld.server_onFixedUpdate( self )
	...
	if not self.challengeStarted and #self.challengeStarters > 0 and not self.loadingWorld then
		for _, challengeStarter in ipairs( self.challengeStarters ) do
			if sm.exists(challengeStarter) then
	...
	if self.challengeStarted and not self.challengeCompleted and #self.challengeFinishers > 0 and not self.loadingWorld then
		for _, challengeFinisher in ipairs( self.challengeFinishers ) do
			if sm.exists(challengeFinisher) then
...
function ChallengeWorld.server_onCellCreated( self, x, y )
	...
	if x == 0 and y == 0 and self.loadingWorld then
...
function ChallengeWorld.server_onCellCreatedFinish( self, x, y )
	...
	if _G.g_tutorial then
		self.tutorialIntroTimer = 0
		self.tutorialStage = "Intro"
	end

	-- Recreate dropped items
	if _G.g_droppedItems ~= nil then
		for _, droppedItem in ipairs( _G.g_droppedItems ) do
...
function ChallengeWorld.server_loadSavedInventory( self, player )
	...
	if _G.g_inventoriesPlayMode[player.id] == nil then
		local inventoryList = {}
		...
		_G.g_inventoriesPlayMode[player.id] = inventoryList
	end

	-- Add to inventory
	local savedInventory = _G.g_inventoriesPlayMode[player.id]

	if savedInventory then
		sm.container.beginTransaction()
		local inv = player:getInventory()
		for i, slot in ipairs( savedInventory ) do
			sm.container.setItem(inv, i - 1, slot["uuid"], slot["quantity"] ) -- container is 0 indexed
		end
...
function ChallengeWorld.server_loadWorldContent( self, data )
	...
	--Creations
	if _G.g_savedCreations ~= nil then -- _G.g_savedCreations can be nil if the challenge is started with the ChallengeStarter and nothing has been built
		for _, creation in ipairs( _G.g_savedCreations ) do
	...
	--Chests
	sm.container.beginTransaction()
	for index, savedChest in ipairs( _G.g_savedChests ) do
		...
		for index, chest in ipairs(self.chests) do
			...
			if sm.exists( chest ) then
				...
				for slot = 1, container:getSize() do
					...
					_G.g_savedChests[index][slot] = { uuid = tostring( item.uuid ), quantity = item.quantity }
				end
			end
		end
	end

	print("_G.g_savedChests 0", _G.g_savedChests)
...
function ChallengeWorld.server_setHideAndSeekOptions(self, data)
...
function ChallengeWorld.server_startChallenge( self )
	...
	if self.hideandseekoptions then
		...
		self.network:sendToClients("client_updateOptions", self.hideandseekoptions)
	end
...
function ChallengeWorld.client_updateOptions(self, data)
	self.hideandseekoptions = data
	if self.hideandseekoptions.options.hiderscansee then
		for _,player in pairs(self.hideandseekoptions.players) do
			if player.name == sm.localPlayer.getPlayer():getName() then
				if player.state == PlayerStates.Seeker then
					for _,rplayer in pairs(self.hideandseekoptions.rplayer_list) do
						rplayer:getCharacter():setNameTag( "" )
					end
				end
				break
			end
	...
	if self.hideandseekoptions.options.hideallname then
		for _,rplayer in pairs(self.hideandseekoptions.rplayer_list) do
			rplayer:getCharacter():setNameTag( "" )
		end
	end
...
function ChallengeWorld.server_onProjectile( self, hitPos, hitTime, hitVelocity, _, attacker, damage, userData, hitNormal, target, projectileUuid )
	...
	if userData and userData.lootUid then
		...
		table.insert( _G.g_droppedItems, droppedItem )
...
function ChallengeWorld.client_onUpdate( self, dt )
	if self.waitingForDeath == true then return end
	if _G.g_effectManager ~= nil then
		_G.g_effectManager:cl_onWorldUpdate( self )
	end
...
function ChallengeWorld.client_onCellLoaded( self, x, y )
	...
	if _G.g_effectManager ~= nil then
	_G.g_effectManager:cl_onWorldCellLoaded( self, x, y )
	end
end
...
function ChallengeWorld.client_onCellUnloaded( self, x, y )
	...
	if _G.g_effectManager ~= nil then
	_G.g_effectManager:cl_onWorldCellUnloaded( self, x, y )
	end
end
...
function ChallengeWorld.client_spawned( self, params )
	...
	if params.player == sm.localPlayer.getPlayer() then
		...
		if _G.g_effectManager and params.playCutscene then
			_G.g_effectManager:cl_playNamedCinematic( "LevelIntro" )
		end
	end
end
