...
function InitializeChallengeGame()
	...
	function ChallengeGame.server_onCreate(self)
		...
		_G.g_disableScrapHarvest = true

		_G.g_unitManager = UnitManager()
		_G.g_unitManager:sv_onCreate()
		...
		if self.data.startLevelIndex ~= nil then
			...
			pcall(
				function()
					sm.game.bindChatCommand("/swim", {}, "server_onChatCommand", "Toggle swim")
				end
			)
		end
	...
	function ChallengeGame.server_onFixedUpdate(self)
		if self.build then
			if self.build.saveReminderTick == nil then
				self.build.saveReminderTick = 9999
			end
		...
		_G.g_unitManager:sv_onFixedUpdate()
	end
	...
	function ChallengeGame.server_startTest(self, levelData)
		...
		for i, player in ipairs(sm.player.getAllPlayers()) do
			...
			_G.g_inventoriesBuildMode[player.id] = inventoryList
		end
	...
	function ChallengeGame.server_stopTest(self)
		...
		sm.event.sendToWorld(self.activeWorld, "server_spawnCharacter", {players = sm.player.getAllPlayers(), inventory=_G.g_inventoriesBuildMode})
	end

	function ChallengeGame.server_start(self)
		_G.g_savedCreations = {}
		_G.g_inventoriesPlayMode = {}
		_G.g_savedChests = {}
		_G.g_tutorial = false
		_G.g_droppedItems = {}
	...
	function ChallengeGame.server_onPlayerJoined(self, player, isNewPlayer)
		...
		if self.activeWorld then
			...
			elseif self.server_challengeStarted then
				...
				self.network:sendToClients(
					"client_onChallengeStarted",
					{ticksSinceStart = sm.game.getCurrentTick() - self.timerStartTick}
				)
			end
		end

		_G.g_unitManager:sv_onPlayerJoined(player)
	end
	...
	function ChallengeGame.server_onSaveLevel(self)
		if self.activeWorld and self.build then
			sm.event.sendToWorld(self.activeWorld, "server_export", self.data)
			self.build.saveReminderTick = sm.game.getCurrentTick()
		end
	end
	...
	function ChallengeGame.server_onFinishPressed(self, params)
		if
			not self.server_challengeCompleted or self.server_sessionID ~= params.sessionID or self.loadingLevelFlag or
				not self.finalLevel
		 then
			return
		end
	...
	function ChallengeGame.server_nextLevel(self)
		...
		if self.play.currentLevelIndex < #self.play.levelList then
			print(
				"Time to change level! (from " .. self.play.currentLevelIndex .. " to " .. (self.play.currentLevelIndex + 1) .. ")"
			)
	...
	function ChallengeGame.server_loadLevel(self, loadJsonData, loadSaveData)
		...
		if self.build == nil or self.build.testing then
			...
			sm.game.setEnableAmmoConsumption(
				enableAmmoConsumption == nil and self.enableAmmoConsumption or enableAmmoConsumption
			)
			local enableFuelConsumption = getSettingValue(levelData.settings, "enable_fuel_consumption")
			sm.game.setEnableFuelConsumption(
				enableFuelConsumption == nil and self.enableFuelConsumption or enableFuelConsumption
			)
		end
	...
	function ChallengeGame.server_loadJsonData(self)
		...
		if levelData.startCreations then
			for _, creation in ipairs(levelData.startCreations) do
				...
				_G.g_savedCreations[#g_savedCreations + 1] = blueprintJson
			end
		end
		...
		if levelData.chestContents then
			_G.g_savedChests[1] = {}
			_G.g_savedChests[1] = levelData.chestContents
		end

		print("_G.g_savedChests", _G.g_savedChests)

		_G.g_tutorial = levelData.tutorial
		_G.g_droppedItems = {}
	end
	...
	function ChallengeGame.server_loadSaveData(self)
		...
		if saveData and saveData.creations then
			_G.g_savedCreations = saveData.creations
			...
			for shape in pairs(availableShapes) do
				if availableShapes[shape] > 0 then
					_G.g_savedChests[1][index] = {uuid = shape, quantity = availableShapes[shape]}
					index = index + 1
				end
			end
			print("_G.g_savedChests 1", _G.g_savedChests)
		end
	end

	function ChallengeGame.server_onCellLoadComplete(self, data)
		--print("GAME::server_onCellLoadComplete::", sm.exists(data.world), data)
		if sm.challenge.isChallengeWorld(data.world) and sm.exists(data.world) then
	...
	function ChallengeGame.server_onChallengeStarted(self)
		--print("server_onChallengeStarted")
		self.server_challengeStarted = true
		self.server_challengeCompleted = false

		self.challengeStartTick = sm.game.getCurrentTick()
		if not sm.exists(self.activeWorld) then
			local completed_worlds = {}
			for _, player in pairs(sm.player.getAllPlayers()) do
				if player.character ~= nil then
					local world = player.character:getWorld()
					if sm.exists(world) and completed_worlds[world.id] == nil then
						completed_worlds[world.id] = true
						sm.challenge.start(world)
						sm.event.sendToWorld(world, "server_challengeStarted")
					end
				end
			end
		else
	...
	function ChallengeGame.server_onChallengeCompleted(self, victoryParams)
		...
		if self.play and victoryParams.canSaveCreation == true then
			saveData = {creations = _G.g_savedCreations}
		end
		...
		if not sm.exists(self.activeWorld) then
			...
			for _, player in pairs(sm.player.getAllPlayers()) do
				if player.character ~= nil then
					...
					if sm.exists(world) and completed_worlds[world.id] == nil then
						completed_worlds[world.id] = true
						sm.event.sendToWorld(world, "server_challengeCompleted")
					end
				end
			end
		else
		...
		if victoryParams.showWinnerName == true then
			self.network:sendToClients(
				"client_onChallengeCompleted",
				{
					time = self.server_completionTime,
					finalLevel = isFinalLevel(self.play),
					winnerName = victoryParams.winnerName,
					newRecord = newRecord
				}
			)
		else
	...
	function ChallengeGame.client_onCreate(self)
		...
		_G.g_effectManager = EffectManager()
		_G.g_effectManager:cl_onCreate()
		...
		if self.data and self.data.startLevelIndex == nil and self.build == nil then
			self.build = {}
		end
		...
		if _G.g_unitManager == nil then
			assert(not sm.isHost)
			_G.g_unitManager = UnitManager()
		end
		_G.g_unitManager:cl_onCreate()

		-- Survival HUD, for HP (TODO: challengeHUD)
		if _G.g_survivalHud == nil or not sm.exists(_G.g_survivalHud) then
			_G.g_survivalHud = sm.gui.createSurvivalHudGui()
		end
		--assert(g_survivalHud)
		_G.g_survivalHud:setVisible("FoodBar", false)
		_G.g_survivalHud:setVisible("WaterBar", false)
	end
	...
	function ChallengeGame.client_onLoadingScreenLifted(self)
		_G.g_effectManager:cl_onLoadingScreenLifted()
	end
		...
		if self.finalLevel == true then
			if self.build then
				...
				if sm.challenge.isMasterMechanicTrial() then
					...
					self.network:sendToServer("server_closeGui")
				end
			end
		else
	...
	function ChallengeGame.client_onChallengeStarted(self, params)
		...
		if sm.isHost then
			_G.g_showMasterMechanicTrialsReward =
				not sm.localPlayer.isGarmentUnlocked(sm.uuid.new("420ea3ba-f09b-44ce-9e4c-7040ef057a0c")) -- Check for stuntman torso
		end
	end
	...
	function ChallengeGame.server_closeGui(self)
		self.network:sendToClients("_client_onDestroy")
		sm.gui.exitToMenu()
	end
end
